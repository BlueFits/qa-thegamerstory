...
exports.addHistoryTitle = async (req, res, next) => {
    ...
    await Hub.findOne({ name: hubName }).exec(async (err, hub) => {
        ...
        if (!hub) {
            ...
            await Hub.findByIdAndUpdate(hub._id, update, { new: true }, (err, result) => {
                if (err) {return next(err);}
...
exports.deleteHistoryTitle = async (req, res, next) => {
    const { title, hubName } = req.body;
    await Hub.findOne({ name: hubName }).exec(async (err, hub) => {
        if (err) {return next(err);}
        if (!hub) {
            ...
            const update = {
                $set: {
                    history: hub.history.filter(item => item !== title),
                },
            };
            await Hub.findByIdAndUpdate(hub._id, update, { new: true }, (err, result) => {
                if (err) {return next(err);}
...
exports.removeHistoryTitle = async (req, res, next) => {
    const { title, hubName } = req.body;
    await Hub.findOne({ name: hubName }).exec(async (err, hub) => {
        if (err) {return next(err);}
        if (!hub) {
